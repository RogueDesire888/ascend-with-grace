...
import { yogaItems } from "@/components/platform/data";
import { searchAll, type SearchHit } from "@/lib/yoga-data";
  ...
  head: () => ({
    meta: [
      { title: "Yoga — Ascend" },
      {
        name: "description",
        content:
          "An interactive yoga encyclopedia: asanas, philosophy, anatomy, mastery path, and tools.",
      },
    ],
  }),
...
function YogaLayout() {
  ...
  const isRoot = location.pathname === "/yoga" || location.pathname === "/yoga/";

  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <Link to="/yoga/start-here" className="inline-flex items-center gap-2">
          <Flower2 className="h-5 w-5 text-orchid-glow" />
...
function RootRedirect() {
  return (
    <div className="rounded-3xl border border-border/50 bg-card/40 p-10 text-center">
      <h1 className="text-3xl font-bold text-foreground">The Yoga Encyclopedia</h1>
      <p className="mx-auto mt-3 max-w-prose text-muted-foreground">
        Choose a section above. New here? Begin with{" "}
        <Link to="/yoga/start-here" className="text-orchid-glow underline">
          Start Here
        </Link>
        .
      </p>
...
function BeltDisplay() {
  const { progress } = useYogaProgress();
  ...
  return (
    <div className="hidden items-center gap-2 rounded-full border border-border/60 bg-card/50 px-3 py-1.5 text-xs sm:inline-flex">
      ...
      <span className="text-muted-foreground">
        {asanaCount} asanas · {goalCount} goals
      </span>
    </div>
  );
}
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-xl">
        ...
        <input
          autoFocus
          placeholder="Try 'warrior', 'pranayama', 'chakra'…"
          value={q}
