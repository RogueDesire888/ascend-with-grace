...
function SanctuaryPage() {
  return (
    <PageFrame
      ...
      title="Your peaceful command center for becoming brighter every day."
    >
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
        <section className="rounded-[2rem] border border-border/70 bg-card/75 p-6 shadow-[var(--shadow-soft)]">
          <AvatarOrb />
          <div className="mt-4 text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-spirit">
              Spirit Avatar
            </p>
            ...
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="text-2xl font-bold text-foreground">
                  Keep the flame soft and steady.
                </h2>
              </div>
          ...
          <div className="mt-6 grid gap-4">
            <div className="rounded-3xl border border-border/70 bg-background/45 p-5">
              ...
              <div className="mt-4 flex gap-3">
                {["bg-spirit", "bg-water", "bg-fire", "bg-earth", "bg-air"].map((item) => (
                  <span
                    key={item}
                    className={`h-9 w-9 rounded-full border border-border ${item}`}
                  />
                ))}
              </div>
            </div>
            <div className="rounded-3xl border border-border/70 bg-background/45 p-5">
              ...
              <div className="mt-4 grid grid-cols-3 gap-3">
                {["Focus", "Balanced", "Quest"].map((item) => (
                  <span
                    key={item}
                    className="rounded-2xl bg-secondary px-3 py-3 text-center text-sm font-semibold text-secondary-foreground"
                  >
                    {item}
                  </span>
                ))}
